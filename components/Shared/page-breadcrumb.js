...
class PageBreadcrumb extends Component {
    render() {
        ...
        return (
            <React.Fragment>
                <section className="bg-half-170 bg-light d-table w-100" style={{ background: `url(${bg1.src}) center center` }}>
                                    ...
                                    <ul className="page-next d-none bg-white shadow p-2 pl-4 pr-4 rounded mb-0">
                                        {
                                            this.props.pathItems.map((item, key) =>
                                                item.id !== length
                                                ? <li key={key}><Link href={item.link}><a  className="text-normal font-weight-bold text-dark">{item.name}</a></Link></li>
                                                : <li key={key}>
